...
public class BookCrudServiceImpl implements BookCrudService {
    ...
    public void showBookById(long id) {
        bookRepository.findById(id).ifPresent(book -> ioService.printString(stringFormatter.getString(book)));
    }

    @Transactional(readOnly = true)
    @Override
    public void findAllBooks() {
        bookRepository.findAll().forEach(book -> ioService.printString(stringFormatter.getString(book)));
    }
